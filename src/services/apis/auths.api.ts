...
import type { IForgotPasswordForm, ILoginForm, IRegisterForm, IVerifyOtpForm } from '@social/types/auths.type';
...
export const callApiForgotPassword = (email: string) => {
  return axios.post<IBackendRes<{ email: string }>>('/api/v1/auths/forgot-password', { email });
};

export const callApiVerifyOtp = (data: IVerifyOtpForm) => {
  return axios.post<IBackendRes<{ email: string }>>('/api/v1/auths/verify-otp', data);
};

export const callApiResetPassword = (data: IForgotPasswordForm) => {
  return axios.post<IBackendRes<{ email: string }>>('/api/v1/auths/reset-password', data);
};
